...
static const char *strDNSSeed[][2] = {
    {"120.27.100.4", "120.27.100.4"},
		 {"121.42.185.200", "121.42.185.200"},
		 {"112.74.92.94", "112.74.92.94"}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SpeedCoin is probably already running."), addrBind.ToString().c_str());
        else
